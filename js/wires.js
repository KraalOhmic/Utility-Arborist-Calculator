// ── SYNC WIRE INPUTS → ARRAY ──
        ...
        function applySagPreset(ft) {
            ...
            renderWireList();
            updateEffHt();
            if (typeof syncMdrFromSagWind === 'function') syncMdrFromSagWind();
        ...
        // ── NEXT TREE ──
        function nextTree() {
            // Clear tree-specific inputs, keep wires/sag/species/wind intact
            ['hd', 'th'].forEach(id => {
                const el = document.getElementById(id);
                if (el) { el.value = ''; el.classList.remove('neg'); }
            });
            // Reset VD to 0
            const vd = document.getElementById('vd');
            if (vd) { vd.value = '0'; vd.classList.remove('neg'); }
            // Reset lean
            const lean = document.getElementById('lean-angle');
            if (lean) { lean.value = 0; updateLean(0); }
            // Clear span helper
            ['span-length', 'span-dist'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.value = '';
            });
            const tResult = document.getElementById('span-t-result');
            if (tResult) tResult.textContent = 'Enter span length and distance to compute t';
            // Hide results
        ...
        // ── SPAN HELPER ──
        function toggleSpanHelper() {
            const wrap = document.getElementById('span-helper-wrap');
            const btn = document.getElementById('span-helper-toggle');
            if (!wrap) return;
            const open = wrap.style.display !== 'none';
            wrap.style.display = open ? 'none' : 'block';
            if (btn) {
                btn.style.background = open ? 'transparent' : 'rgba(59,139,255,.12)';
                btn.style.borderColor = open ? 'rgba(59,139,255,.25)' : 'rgba(59,139,255,.4)';
                btn.style.color = open ? 'var(--text2)' : 'var(--accent2)';
            }
            if (!open) document.getElementById('span-length')?.focus();
        }

        function calcSpanT() {
            const span = parseFloat(document.getElementById('span-length')?.value);
            const dist = parseFloat(document.getElementById('span-dist')?.value);
            const result = document.getElementById('span-t-result');
            const posEl = document.getElementById('span-pos');
            if (!result) return;
            if (!Number.isFinite(span) || span <= 0 || !Number.isFinite(dist) || dist < 0) {
                result.textContent = 'Enter span length and distance to compute t';
                result.style.color = 'var(--text2)';
                return;
            }
            if (dist > span) {
                result.textContent = 'Distance cannot exceed span length';
                result.style.color = 'var(--warn)';
                return;
            }
            const raw = dist / span;
            const t = Math.max(0.1, Math.min(0.9, Math.round(raw * 10) / 10));
            result.textContent = `t = ${t.toFixed(1)} (${Math.round(raw * 100)}% of span)`;
