...
function syncMdrFromSagWind() {
    ...
    // Pull sag directly from wire configuration — use worst (max) non-neutral sag
    let sagCurrent = 0;
    if (typeof wires !== 'undefined' && wires.length) {
        const sagVals = wires
            .filter(w => !/neutral/i.test(String(w.name || '')))
            .map((w, i) => {
                const sagEl = document.getElementById('w-sag-' + wires.indexOf(w));
                return sagEl ? parseFloat(sagEl.value) || 0 : (w.sag || 0);
            })
            .filter(v => Number.isFinite(v));
        if (sagVals.length) sagCurrent = Math.max(...sagVals);
    }
    // Fallback to lastSag if available
    if (sagCurrent === 0 && typeof lastSag !== 'undefined' && lastSag && Number.isFinite(Number(lastSag.sagNow))) {
        sagCurrent = Number(lastSag.sagNow);
    }

    // Worst sag from lastSag thermal projection if available, otherwise use current
    let sagWorst = sagCurrent;
    if (typeof lastSag !== 'undefined' && lastSag && Number.isFinite(Number(lastSag.worstSagMax))) {
        sagWorst = Number(lastSag.worstSagMax);
    }

    const extraSag = Math.max(0, sagWorst - sagCurrent);
    sagCurrentEl.value = sagCurrent.toFixed(2);
