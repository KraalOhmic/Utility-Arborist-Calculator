        ...
        function runCheck() {
            ...
            document.getElementById('banner-sub').textContent = overallClear
                ? `Worst margin: +${worstMargin.toFixed(1)} ft (all wire minimums met)`
                : overallWarn
            ...
            document.getElementById('wire-results').innerHTML = results.map(r => {
              ...
              <div class="res-metrics">
                ...
                <div class="rmet"><div class="rmet-lbl">Margin</div><div class="rmet-val" style="color:${r.clear ? 'var(--success)' : (r.margin >= 0 ? 'var(--warn)' : 'var(--danger)')}">${r.margin >= 0 ? '+' : ''}${r.margin.toFixed(1)}<span class="rmet-unit">ft</span></div></div>
              </div>
            </div>`;
            }).join('');
